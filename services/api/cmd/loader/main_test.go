...
func TestAtlasModelsMatchServerAutoMigrateModels(t *testing.T) {
	...
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("atlas model list drifted from server AutoMigrate list\nwant: %#v\n got: %#v", want, got)
	}
}
...
func TestAtlasCustomPostgresSchemaPreservesRuntimeInvariants(t *testing.T) {
	...
	for _, want := range []string{
		"CREATE TYPE user_role AS ENUM ('viewer', 'streamer', 'admin', 'agency')",
