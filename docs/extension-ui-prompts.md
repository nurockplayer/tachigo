...
> 範圍：僅針對小尺寸 extension panel UI，不涵蓋 dashboard，也不宣告產品 runtime 已完成遷移。

---

## 1. 產品背景

> **注意：** 這份文件是 UI 提示詞參考，不是產品定位或 runtime migration 的 source of truth。現況文件若仍以 `Twitch Extension`、`extension_jwt` 等術語描述既有流程，應以 repo 目前已核准的 architecture / design 文件為準。
...
- [apps/extension/src/App.tsx](../apps/extension/src/App.tsx)
