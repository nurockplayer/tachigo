...
- 不在沒有 source issue 的情況下實作；先找既有 issue，沒有才依 repo 規則開新 issue。
...
- 不修改遠端 issue / PR / label / review / branch，除非使用者已明確授權該公開狀態變更。
...
3. 若沒有合適 issue，依 repo 規則建立 source issue。
...
- 在 source issue 留 proof comment。
- close source issue as completed, unless it is intentionally long-lived tracking / ledger.
