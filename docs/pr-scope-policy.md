...
- 高風險 PR 不進入自動 ready / merge 路徑，必須保留 human review

## PR Risk Class

所有人工或 Codex 產生的 PR body 都必須在 `PR Risk Class` 區塊剛好勾選一項：

| Class | 適用範圍 | auto-ready |
|---|---|---|
| R0 | docs / template / metadata only | 可用 |
| R1 | tests / CI / tooling only | 可用 |
| R2 | frontend behavior | 可用 |
...
- PR body 必須剛好勾選一個 `PR Risk Class`
- `R4` PR 不可使用 `auto-ready`
...
若用 `AUTO_READY=1` 開 PR，本地 preflight 也會擋下 `R4`，避免高風險 PR 在送上 GitHub 前就進入 auto-ready label 路徑。
