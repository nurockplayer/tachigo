...
PR #265 (`[frontend] migrate demo app-shell foundations into tachimint (1/4)`) 將 demo app-shell 的第一批基礎資產搬入 `tachimint/`，包含：
...
2. binary asset 被展開成 `GIT binary patch`，其中 `tachimint/src/assets/fonts/Zpix.ttf` 約 7MB，會讓 diff、review 與 clone 成本失真。
...
新增 `.gitattributes`，先針對 `tachimint/src/assets/` 中常見 binary asset 類型啟用 LFS：

```gitattributes
# Tachimint binary assets
tachimint/src/assets/**/*.png filter=lfs diff=lfs merge=lfs -text
tachimint/src/assets/**/*.jpg filter=lfs diff=lfs merge=lfs -text
tachimint/src/assets/**/*.jpeg filter=lfs diff=lfs merge=lfs -text
tachimint/src/assets/**/*.webp filter=lfs diff=lfs merge=lfs -text
tachimint/src/assets/**/*.gif filter=lfs diff=lfs merge=lfs -text

# Tachimint fonts
tachimint/src/assets/**/*.ttf filter=lfs diff=lfs merge=lfs -text
tachimint/src/assets/**/*.otf filter=lfs diff=lfs merge=lfs -text
tachimint/src/assets/**/*.woff filter=lfs diff=lfs merge=lfs -text
tachimint/src/assets/**/*.woff2 filter=lfs diff=lfs merge=lfs -text
```

先不要一口氣套到所有 product surface，原因是：

- `dashboard/`、`extensions/` 可能有不同資產策略
...
- `tachimint/src/assets/242a2b8162b4542ca6839e84ad45ad4a36c0257c.png`
- `tachimint/src/assets/fonts/PressStart2P-Latin.woff2`
- `tachimint/src/assets/fonts/Zpix.ttf`
...
```bash
git add --renormalize tachimint/src/assets
...
```bash
git show HEAD:tachimint/src/assets/fonts/Zpix.ttf | sed -n '1,5p'
```
