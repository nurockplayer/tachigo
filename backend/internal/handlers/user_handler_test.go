...
func handlerSIWEMessage(address, nonce string) string {
	return fmt.Sprintf(
...
func seedHandlerWalletNonce(t *testing.T, env *testEnv, address, nonce string) {
	t.Helper()
	if err := env.db.Create(&models.Web3Nonce{
		Nonce:     nonce,
...
func TestLinkWalletHandler_Success(t *testing.T) {
	...
	msg := handlerSIWEMessage(strings.ToLower(addr), nonce)
...
func TestLinkWalletHandler_WalletAlreadyLinked(t *testing.T) {
	...
	seedHandlerWalletNonce(t, env, addr, nonceA)
	msgA := handlerSIWEMessage(strings.ToLower(addr), nonceA)
	...
	seedHandlerWalletNonce(t, env, addr, nonceB)
	msgB := handlerSIWEMessage(strings.ToLower(addr), nonceB)
