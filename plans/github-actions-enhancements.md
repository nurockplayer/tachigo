...
### 2. Draft PR auto-ready（workflow 已完成於 PR B；rollout 驗證中）
...
- `.github/workflows/ci.yml` 的 `auto-ready-after-ci` job，讓同一個 PR 的 CI
  gate 結束後能立即重查 live branch protection 並觸發 ready transition。
...
- 排除 `dependabot[bot]`；若未來導入 Renovate，需先把 `renovate[bot]` 加入 deny-list。
- 重新查 live PR head SHA，不信任 stale event payload。
- 透過 GraphQL `branchProtectionRule` 讀取 live required checks 作為 readiness gate。
...
- CI completion hook 只在 `scope-gate`、`backend-ci`、`frontend`、`dashboard`、`contracts`
  都是 `success` 或 `skipped` 後執行，且仍以 live branch protection required checks 作為最後 gate。

PR B 後續 review 決策改為直接讀 live branch protection required checks；因此下表目前作為背景紀錄與人工核對用，不是 workflow 內的 hard-coded config。以下清單以 2026-05-04 重新查詢 GitHub branch protection API 的 live required contexts 為準：
...
- 本 rollout PR 也補上 CI completion hook；PR #488 是第一個實際驗收候選。
...
| `ci.yml` | `pull-requests: write` / `checks: read` / `statuses: read` | 只限 `auto-ready-after-ci` job；只跑 `pull_request`；必須等 protected CI gate jobs 完成，並驗證同 repo draft PR、`auto-ready` label、非 dependency bot、live head SHA 與 required checks 全部成功 |
...
| `auto-ready-pr.yml` | `pull-requests: write` / `checks: read` / `statuses: read` | 只處理同 repo draft PR、`auto-ready` label、非 dependency bot、live head SHA 與 required checks 全部成功 |
...
### PR A：Dependabot directory drift（已完成於 PR A）
...
### PR B：Auto-ready workflow（已完成於 #472；rollout 由本 PR 收尾）
...
- CI workflow regression tests 覆蓋 `auto-ready-after-ci` job 的 needs、permissions、eligibility guard 與 live branch-protection gate。
