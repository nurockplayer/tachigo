...
### 2. Draft PR auto-ready（workflow 已完成於 PR B；rollout 中）
...
- 剩餘驗收：下一個真實 Codex-authored PR 應驗證 draft + `auto-ready` label + CI 全綠後自動 ready。
...
- 下一個 Codex task PR 使用 draft + `auto-ready` label 驗證實際 ready transition。
