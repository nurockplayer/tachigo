...
test('auto-ready workflow checks required contexts and excludes its own run', async () => {
  ...
  assert.match(workflow, /const requiredSuccessConclusions = new Set\(\['success', 'neutral', 'skipped'\]\)/)
  ...
  assert.match(workflow, /if \(!observed\.has\(name\)\) \{\s+observed\.set\(name, checkRunPassed\(run, requiredSuccessConclusions\)\)\s+\}/)
